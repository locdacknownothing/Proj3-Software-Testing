...
class EditProfileUtils(Utils):
    def init(self):
        super().init()
        self.error_id = ["id_error_firstname",
                         "id_error_lastname", "id_error_email"]
        self.error_message = [
            "Missing given name", "Missing last name", "Required", "Invalid email address"]
    ...
    def getEditProfile(self):
        ...
        edit_profile = wait.until(EC.visibility_of_element_located(
            (By.PARTIAL_LINK_TEXT, "Edit profile")))
        edit_profile.click()
    ...
    def updateProfile(self):
        try:
            ...
            save_changes = wait.until(EC.visibility_of_element_located(
                (By.XPATH, "//input[@type='submit' and @value='Update profile']")))
    ...
    def isSuccess(self):
        try:
            ...
            cont = wait.until(EC.visibility_of_element_located(
    ...
    def checkEmailChange(self):
        try:
            wait = WebDriverWait(self.driver, 2)
            cancel = wait.until(EC.visibility_of_element_located(
                (By.PARTIAL_LINK_TEXT, "Cancel email change")))
        except TimeoutException:
            return
...
class EditProfileTestCase(unittest.TestCase, EditProfileUtils):
    ...
    def test_002_012(self):
        ...
        expected = self.isError(0, 0) and self.isError(
            1, 1) and self.isError(2, 2)
        self.driver.quit()
        assert expected
